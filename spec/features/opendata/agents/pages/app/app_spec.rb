...
describe "opendata_agents_pages_app", dbscope: :example do
  ...
  let!(:node) { create :opendata_node_app, name: "opendata_app", layout_id: layout.id }
  let!(:node_member) { create :opendata_node_member, layout_id: layout.id }
  let!(:node_mypage) { create :opendata_node_mypage, filename: "mypage", layout_id: layout.id }
  let!(:category) { create :opendata_node_category, basename: "opendata_category1", layout_id: layout.id }
  let!(:area) { create :opendata_node_area, basename: "opendata_area_1", layout_id: layout.id }
  let!(:node_search) { create :opendata_node_search_app, layout_id: layout.id }

  let!(:node_login) { create :member_node_login, redirect_url: node.url, layout_id: layout.id }
  ...
  context "show url app" do
    let!(:node_search_dataset) { create(:opendata_node_search_dataset, layout_id: layout.id) }
    let(:node_ds) { create :opendata_node_dataset, basename: "opendata_dataset1", layout_id: layout.id }
    let(:dataset) { create(:opendata_dataset, node: node_ds, layout_id: layout.id) }
    let(:app) do
    ...
    context "with default options" do
      it do
        ...
        expect(page).not_to have_css(".side-right .download-all")
        ...
        expect(page).to have_css(".detail #tabs .tab-body #tab-dataset")
        expect(page).to have_css(".detail #tabs .tab-body #tab-idea")
        expect(page).not_to have_css(".detail .author dt", text: "実行回数")
    ...
    context "when point is hide" do
      ...
      it do
        ...
        expect(current_path).to eq app.url
        expect(page).to have_css("h1.name", text: app.name)
        expect(page).not_to have_css(".side-right .point")
        expect(page).to have_css(".categories")
      end
    end
    ...
    context "when dataset is disabled" do
      ...
      it do
        ...
        expect(page).to have_css("h1.name", text: app.name)
        expect(page).to have_css(".detail #tabs .names #url")
        expect(page).not_to have_css(".detail #tabs .names #dataset")
        expect(page).to have_css(".detail #tabs .names #idea")
        expect(page).to have_css(".detail #tabs .tab-body #tab-url")
    ...
    context "when idea is disabled" do
      ...
      it do
        ...
        expect(page).to have_css(".detail #tabs .names #url")
        expect(page).to have_css(".detail #tabs .names #dataset")
        expect(page).not_to have_css(".detail #tabs .names #idea")
        expect(page).to have_css(".detail #tabs .tab-body #tab-url")
        expect(page).to have_css(".detail #tabs .tab-body #tab-dataset")
  ...
  context "show html app" do
    ...
    context "with default options" do
      it do
        ...
        expect(page).to have_css(".detail #tabs .tab-body #tab-html")
        expect(page).to have_css(".detail #tabs .tab-body #tab-css")
        expect(page).to have_css(".detail #tabs .tab-body #tab-js")
        expect(page).to have_css(".detail #tabs .tab-body #tab-sample")
        expect(page).to have_css(".detail #tabs .tab-body #tab-dataset")
        expect(page).to have_css(".detail #tabs .tab-body #tab-idea")
